...
if(!isset($_SESSION["usuario"])){
    ...
    if($_SERVER["REQUEST_METHOD"]=="POST"){
        ...
        if(!empty($_POST["codigo"])){
            $id_producto = htmlspecialchars($_POST["codigo"]);
        }else{
        ...
        if(!empty($_POST["peso"])){
            ...
            $hay_errores = "El campo peso está vacío";
            $errores = true;
        }
        ...
        if(!empty($_POST["stock"])){
            ...
            $hay_errores = "El campo stock está vacío";
            $errores = true;
        }
        ...
        if(!empty($_POST["categoria"])){
            ...
            $hay_errores = "El campo categoria está vacío";
            $errores = true;
        }
        ...
        if($hay_errores == false){
            try{
                ...
                if($resultado->rowCount()==0){
                    $hay_errores = "El producto no existe";
                    $errores = true;
                }else{
                    //actualizamos los campos de la categoría
                    $actualizacion = "UPDATE productos SET codprod='".$id_producto."', nombre='".$nombre."', descripcion='".$descripcion."', peso='".$peso."', stock='".$stock."', categoria='".$categoria."' WHERE codprod = $id_producto";
                    $resultado = $bd->query($actualizacion);

                    //comprobación de errores
                    if($resultado){
                        $salida = "Actualización de datos del producto " . $nombre . " realizados correctamente.";
                    }else{
